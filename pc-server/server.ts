...
// 对话上下文)+ 本提示词一起发给"提示词优化模型"。模型返回的优化版直接替换输入框,所以
// 输出必须纯净(无前言/解释/引号)。设计目标:把口语化、混乱的草稿打磨成清晰专业的版本,
// 同时严格不改原意、不膨胀简单请求、保留占位符和固定内容。上下文是可选的——首条消息或
// 无对话时省略,此时原则 2 要求模型不要凭空猜测模糊指代(错误的猜测比模糊更糟)。
...
1. **严格保留原意** —— 不增加用户没有提出的诉求,不删减用户已经表达的内容,不擅自改变核心意图。你的职责是打磨表达,不是替用户重新定义需求。如果原文已经清晰完整,原样输出即可,不要为了"优化"而画蛇添足。

2. **消除歧义** —— 用户常用模糊或笼统的表述("弄一下""优化一下""帮我处理那个")。如果下方附带了对话上下文,请结合上下文理解这些指代具体指向什么;如果没有上下文或仍无法确定,保留原表述,不要凭空猜测后替换——错误的猜测比模糊更糟。

3. **专业化用词** —— 把口语化、不精确的说法替换为更准确、更得体的表达,让模糊的动词变成具体的动作。例如:"帮我弄个东西" → 点明具体要做什么;"写个东西给老板" → 明确是邮件 / 汇报 / 请示中的哪一种;"弄好看点" → 指明是调整措辞 / 优化排版 / 精简结构;"翻译一下" → 点明源语言、目标语言、要保留的风格。

4. **适度结构化** —— 当请求包含多个要点(背景、需求、约束、期望的输出格式或语气)时,用分节或编号列表清晰组织。但如果只是一句话的简单请求,保持简洁,不要用多余的框架稀释重点——简洁本身就是专业。

5. **补全隐含信息** —— 如果提示词隐含了目标读者、语气、篇幅、格式或希望 AI 扮演的角色,且能从上下文或常识中合理推断,将其显式写出。例如"写个周报" → 点明是面向团队的工作周报、周频、含本周总结与下周计划。无法合理推断的不要编造。

6. **保持原文语言** —— 中文保持中文,英文保持英文,不要翻译,不要自行添加用户未要求的外语。

7. **保留占位符与固定内容** —— 原文中的模板占位符(如 {{name}}、{topic}、<url>、[日期])、代码块、数据、公式、引用原样保留,不修改、不"改进"。只优化这些固定内容之外的说明性文字。
...
async function routeApi(request: Request, url: URL) {
  ...
  if (path === "prompt/optimize" && request.method === "POST") {
    ...
    if (context) {
      prompt += `\n\n以下是用户当前对话的最近几轮,供你理解提示词中可能引用的人物、话题、术语或场景。请借助它更准确地理解提示词,不要回复或延续这段对话:\n\n<conversation_context>\n${context}\n</conversation_context>`;
    }
    prompt += `\n\n请优化以下提示词,直接输出优化后的版本:\n\n<original_prompt>\n${text}\n</original_prompt>`;
    try {
